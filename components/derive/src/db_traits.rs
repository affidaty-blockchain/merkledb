...
impl ObjectHashStruct {
    pub fn implement_object_hash(&self) -> impl ToTokens {
        ...
        quote! {
            impl merkledb::ObjectHash for #name {
                fn object_hash(&self) -> merkledb::_reexports::Hash {
                    let bytes = merkledb::BinaryValue::to_bytes(self);
                    merkledb::_reexports::hash(&bytes)
                }
            }
        }
    }
...
impl BinaryValueStruct {
    fn implement_binary_value_from_pb(&self) -> proc_macro2::TokenStream {
        ...
        quote! {
            impl merkledb::BinaryValue for #name {
                ...
                ) -> std::result::Result<Self, merkledb::_reexports::Error> {
                    use protobuf::Message as _;
...
impl AccessField {
    ...
    fn constructor(&self, field_index: usize) -> impl ToTokens {
        let from_access = quote!(merkledb::access::FromAccess);
...
impl FromAccess {
    fn access_fn(&self) -> impl ToTokens {
        ...
        quote! {
            fn from_access(
                access: #access_ident,
                addr: merkledb::IndexAddress,
            ) -> Result<Self, merkledb::access::AccessError> {
                #fn_impl
            }
        }
    }
...
impl ToTokens for FromAccess {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let name = &self.ident;
        let tr = quote!(merkledb::access::FromAccess);
        let access_ident = &self.access_ident;
