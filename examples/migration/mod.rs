...
#[derive(ObjectHash, Copy, Clone, Debug, Serialize, Deserialize)]
...
impl BinaryValue for PublicKey {
    ...
    fn from_bytes(bytes: std::borrow::Cow<'_, [u8]>) -> anyhow::Result<Self> {
        let len = std::cmp::min(bytes.len(), PUBLIC_KEY_LENGTH);
        let mut key_bytes: [u8; PUBLIC_KEY_LENGTH] = [0; PUBLIC_KEY_LENGTH];
        key_bytes.clone_from_slice(&bytes[0..len]);
...
impl BinaryKey for PublicKey {
    ...
    fn write(&self, buffer: &mut [u8]) -> usize {
        ...
        buffer.clone_from_slice(&src);
        src.len()
    }

    fn read(buffer: &[u8]) -> Self::Owned {
        let len = std::cmp::min(buffer.len(), PUBLIC_KEY_LENGTH);
        let mut bytes: [u8; PUBLIC_KEY_LENGTH] = [0; PUBLIC_KEY_LENGTH];
        bytes.clone_from_slice(&buffer[0..len]);
        PublicKey::new(bytes)
    }
}
