...
macro_rules! impl_binary_key_for_binary_value {
    ($type:ty) => {
        impl merkledb::BinaryKey for $type {
            fn size(&self) -> usize {
                merkledb::BinaryValue::to_bytes(self).len()
            }

            fn write(&self, buffer: &mut [u8]) -> usize {
                let mut bytes = merkledb::BinaryValue::to_bytes(self);
            ...
            fn read(buffer: &[u8]) -> Self::Owned {
                ...
                <Self as merkledb::BinaryValue>::from_bytes(buffer.into()).unwrap()
            }
        }
    };
}
