...
//! };
//! use merkledb::generic::{ErasedAccess, IntoErased};
//!
...
//!
//! ```
//! use merkledb::{access::AccessExt, Database, TemporaryDB};
//! use merkledb::generic::GenericRawAccess;
//! use std::rc::Rc;
//!
//! let db = TemporaryDB::new();
//! let fork = db.fork();
...
impl GenericRawAccess<'_> {
    ...
    pub fn is_mutable(&self) -> bool {
        matches!(
            self,
            GenericRawAccess::Fork(_) | GenericRawAccess::OwnedFork(_)
        )
    }
}
